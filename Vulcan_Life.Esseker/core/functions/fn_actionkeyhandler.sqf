...
if (isPlayer _curObject && _curObject isKindOf "Man") then {
	...
	if ((cursorTarget getVariable ["restrained",false]) && !dialog && playerSide == independent) then {
        _nil = createDialog "vn_PMenu";
    };
    ...
    if (_isVehicle) then {
	if ((_curObject isKindOf "O_supplyCrate_F") && {player distance _curObject < 3} ) exitWith {
    if (alive _curObject) then {
        [_curObject] call life_fnc_seizeBox;
    };
};
if ((_curObject isKindOf "I_MBT_03_cannon_F" && _curObject getVariable ["randomLocked",true]) && {player distance _curObject < 7} ) exitWith {
    if (alive _curObject) then {
		if([false,"MilitaryCodeBreaker",1] call life_fnc_handleInv)then{
        [_curObject] call legac_fnc_seizeTank;
		}else{
			hint "You Need a Miliary Code Breaker to Picklock this Vehicle";
		};
    };
};
if ((_curObject isKindOf "O_Heli_Attack_02_black_F") && {player distance _curObject < 7} ) exitWith {
    if (alive _curObject) then {
		if([false,"MilitaryCodeBreaker",1] call life_fnc_handleInv)then{
        [_curObject] call legac_fnc_seizeHelo;
		}else{
			hint "You Need a Miliary Code Breaker to Picklock this Vehicle";
		};
    };
};
if(_curObject in ["O_Heli_Attack_02_black_F","I_MBT_03_cannon_F","O_supplyCrate_F"])then{
}else{
        if (!dialog) then {
