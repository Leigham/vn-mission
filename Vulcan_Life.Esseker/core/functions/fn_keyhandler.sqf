...
switch (_code) do {
	...
	case 201: {//Page up
		if(player getVariable ["vnIsBuilding",true])then{
			if(_shift && !_ctrlKey)then{
				vn_currentBuildable setPosATL [getPosATL vn_currentBuildable select 0, getPosATL vn_currentBuildable select 1, (getPosATL vn_currentBuildable select 2) + 0.25];
			};
			...
			if(!_shift && !_ctrlKey)then{
				vn_currentBuildable setPosATL [getPosATL vn_currentBuildable select 0, getPosATL vn_currentBuildable select 1, (getPosATL vn_currentBuildable select 2) + 0.10];
			};
		};
	};
	case 209: {// Page Down
