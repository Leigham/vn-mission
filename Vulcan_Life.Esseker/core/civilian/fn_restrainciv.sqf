...
[] spawn {
    ...
    for "_i" from 0 to 1 step 0 do {
        ...
        if (!([west,getPos player,30] call life_fnc_nearUnits) && (player getVariable ["civrestrained",false]) && isNull objectParent player) exitWith {
            player setVariable ["civrestrained",false,true];
            player setVariable ["civEscorting",false,true];
            player setVariable ["civtransporting",false,true];
            detach player;
