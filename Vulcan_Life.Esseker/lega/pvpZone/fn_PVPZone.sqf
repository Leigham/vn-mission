	...
	if ((_unit distance (getMarkerPos "PVP_Enter")) <= 5 && (alive _unit)) then {
	 ...
	 if (_EE == "Exit" && (alive _unit) && (_unit distance getMarkerPos "PVP_Exit_Marker") <= 5) then {
	 	if ((_unit distance (getMarkerPos "PVP_Exit_Marker")) <= 5 && (alive _unit)) then {
	 		...
	 		while {_timer > 0 && ((_unit distance getMarkerPos "PVP_Exit_Marker") <= 15) && (alive _unit)} do {
	 			cutText  [([_timer, 1, 0, true] call CBA_fnc_formatNumber) + " Seconds Till You Exit the PVP Zone", "PLAIN",0.05];
	 			sleep 1;
	 			_timer = _timer - 1;
				if(_unit distance getMarkerPos "PVP_Exit_Marker" > 15)exitWith{hint "You have strayed too far from the exit point, please request again"};
	 		};
	 		waitUntil {_timer == 0};
			_SuckmyDonkeyone = ["civ_spawn_2","civ_spawn_4","civ_spawn_3"] call BIS_fnc_SelectRandom;
			_postobemoved = getMarkerPos _SuckmyDonkeyone;
			_unit setPos _postobemoved;
		};
	};
};